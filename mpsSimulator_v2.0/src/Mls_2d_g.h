...
#pragma once
#include "Simulator.h"
#include "Particle_2d_g.h"

namespace SIM {

	template <typename real, enum Dim dim>
	class Mls_2d_g : public Simulator < real, dim, Mls_2d_g<real, dim> > {
					...
					for (int j = -1; j <= 1; j++) {
						for (int i = -1; i <= 1; i++) {
							...
							for (unsigned m = 0; m < part->cell->linkList[key].size(); m++) {
								...
								const vec3 npq = part->poly2d_g(dr);
								const vec3 a = part->invMat[p] * (w* npq);
								const vec3 lp = part->poly2d_g_lap_0;
								real pq = lp.dot(a);
								if (part->type[p] == BD1) {
...
#pragma omp parallel for
		...
		void init_() {
			part = new Particle_2d_g<real, dim>();
			part->clean();
			*part << "Geo.in";
