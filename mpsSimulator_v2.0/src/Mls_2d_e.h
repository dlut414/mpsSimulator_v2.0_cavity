...
namespace SIM {
	...
	class Mls_2d_e : public Simulator < real, dim, Mls_2d_e<real, dim> > {
					...
					for (int j = -1; j <= 1; j++) {
						for (int i = -1; i <= 1; i++) {
							...
							for (unsigned m = 0; m < part->cell->linkList[key].size(); m++) {
								...
								const vec5 npq = part->poly2d_e(dr);
								const vec5 a = part->invMat[p] * (w* npq);
								const vec5 lp = part->poly2d_e_lap_0;
								real pq = lp.dot(a);
								if (part->type[p] == BD1) {
...
#pragma omp parallel for
		...
		void init_() {
			part = new Particle_2d_e<real, dim>();
			part->clean();
			*part << "Geo.in";
