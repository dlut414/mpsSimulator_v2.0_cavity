...
#include "header.h"
#include "Simulator.h"
#include "Particle_2d_f.h"

namespace SIM {

	template <typename real, enum Dim dim>
	class Mls_2d_f : public Simulator < real, dim, Mls_2d_f<real, dim> > {
					...
					for (int j = -1; j <= 1; j++) {
						for (int i = -1; i <= 1; i++) {
							...
							for (unsigned m = 0; m < part->cell->linkList[key].size(); m++) {
								...
								const vec8 npq = part->poly2d_f(dr);
								const vec8 a = part->invMat[p] * (w* npq);
								const vec8 lp = part->poly2d_f_lap_0;
								real pq = lp.dot(a);
								if (part->type[p] == BD1) {
...
#pragma omp parallel for
		...
		void init_() {
			part = new Particle_2d_f<real, dim>();
			part->clean();
			*part << "Geo.in";
